...
export async function saveFitData(keyData: any): Promise<void> {
    ...
    try {
        await db.run(
            'INSERT INTO fit_data (start_time, total_elapsed_time, activity_type, total_distance, avg_heart_rate, max_heart_rate, avg_power, max_power, event, event_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [keyData.startTime, keyData.totalElapsedTime, keyData.activityType, keyData.totalDistance, keyData.avgHeartRate, keyData.maxHeartRate, keyData.avgPower, keyData.maxPower, keyData.event, keyData.eventType]
        );
    } catch (error) {
